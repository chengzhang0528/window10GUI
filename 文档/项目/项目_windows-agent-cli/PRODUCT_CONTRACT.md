...
便携分发包使用系统 .NET 10 Desktop Runtime x64。公开原生入口在运行时缺失时从微软官方来源准备运行时，安装可能要求联网和 Windows UAC；已有兼容运行时直接复用，正常调用不额外检测或联网。不把机器级安装成功标记随目录复制，首次缺少运行时的机器不承诺离线即用。安装提示只写 stderr，不消费或污染 Agent 的 stdin/stdout 协议；业务开始后的失败不得通过安装重试重放动作。

dsh 接入是独立版本的薄插件，不改变 CLI 的通用底座定位。插件默认禁用，由 profile 所有者启用；优先复用显式配置或相邻便携包的公开入口。需要下载时，只读取部署配置选定的固定版本完整资产，校验大小、SHA-256、文件清单、许可与安全路径，不搜索最新版本、切换来源或覆盖已有安装。二进制正式分发采用工作空间指定 OSS，GitHub 仅承载源码；没有已发布资产配置时明确提示配置缺失，不宣称 npm 包安装后已经具备 CLI。

插件首次请求先用无桌面副作用的能力探测确认 CLI 可响应，运行时准备期间不发送业务操作。仅原生入口的结构化准备信号启用独立安装预算，安装失败、安装超时、业务超时与无响应退出分别报告。取消准备只结束等待并关闭输入，不主动终止系统安装器；已开始的 Windows 安装可能继续，应核验其结果后再尝试。诊断仅保留有界 stderr 尾部并标记截断，不持久化原文。

## 底座定位与职责边界

Windows Agent CLI 固定为“可被任意 Agent 编排的 Windows 10 交互式桌面自动化执行底座”。它不是端到端智能体、业务自动化产品、脚本语言或测试平台，也不内置语言模型规划和视觉决策；离线 OCR 模型只执行文字识别。闲鱼、表单和其他应用只能作为验证底座能力的场景；场景数量增加时，优先沉淀可跨应用复用的执行原语，不把场景规则写入底座。
...
- 桌面文字主识别使用随应用交付的 Paddle Tiny，运行时离线读取嵌入模型；不接入 Small。Windows OCR 仅在调用方请求词框时辅助定位，不替换 Tiny 正文。跨引擎词框必须来自同一截图且通过文字与几何对应校验；无法可靠对应时返回空词框及定位状态，不按字数均分行框、不猜测行内点击位置。Windows 语言包缺失只使辅助定位不可用；Tiny 初始化或识别失败仍明确报错，不静默降级。
...
- `profile_mode=auto`（默认）先发现已有 CDP，没有可用端点时启动受控 profile，不关闭现有 Chrome。`profile_mode=current` 仅在没有运行中 Chrome 时尝试当前用户目录启动，不自动关闭窗口，失败返回 `CHROME_CURRENT_PROFILE_UNAVAILABLE`；`profile_mode=managed` 只复用受控 profile 的端点或启动受控实例，不扫描普通端口。受控 profile 默认位于 `%LOCALAPPDATA%\WindowsAgent\ChromeProfile`，也可通过 `user_data_dir` 指定。
...
- 不承诺第一版提供应用启动、进程管理、Vision、WGC、拖拽、任意图像理解、应用语义消息解析、安装器或自动更新；离线 OCR 只提供定位文本候选，识别成功不等于真实点击或业务成功。
